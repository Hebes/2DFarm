...
namespace Core
{
    public class CoreData : ICore
    {
        ...
        private void InitData<T>() where T : IData
        {
            var package = YooAssets.GetPackage(ConfigCore.YooAseetPackage);
            string fullName = typeof(T).FullName;

            var handle = package.LoadRawFileSync(fullName);
            //RawFileHandle handle = package.LoadRawFileAsync(fullName);
            ...
            if (handle.Status == EOperationStatus.Succeed)
            {
                ...
                if (bytesDataDic.ContainsKey(fullName))
                    bytesDataDic[fullName] = itemDetailsList;
                bytesDataDic.Add(fullName, itemDetailsList);
            }
        }
        ...
        public T GetDataOne<T>(int id) where T : class, IData
        {
            IData data = null;
            string fullName = typeof(T).FullName;
            if (bytesDataDic.TryGetValue(fullName, out List<IData> value))
                data = value.Find(data => data.GetId() == id);
            return data as T;
        }
