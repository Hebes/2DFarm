...
namespace ACFrameworkCore
{
    public class StateMachineSystem
	{
		...
		public void AddNode(IStateNode stateNode)
		{
			...
			{
                ACDebug.Error($"State node already existed : {nodeName}");
			}
		}
		...
		public void ChangeState(string nodeName)
		{
			...
			if (node == null)
			{
                ACDebug.Error($"Can not found state node : {nodeName}");
				return;
			}

			ACDebug.Log($"{_curNode.GetType().FullName} --> {node.GetType().FullName}");
			_preNode = _curNode;
			_curNode.OnExit();
			_curNode = node;
			_curNode.OnEnter();
